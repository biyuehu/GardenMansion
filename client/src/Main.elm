...
type Msg
  ...
  | SubmitLogin
  | GotLoginResult (Result Http.Error ResLoginApi)
  | Logout
  ...
  | GotMessages (Result Http.Error ResMessageApi)
  ...
  | GotExpenses (Result Http.Error ResExpenseApi)
  ...
  | GotUsers (Result Http.Error ResUserApi)
  ...
  case msg of
    ...
    GotUsers result ->
      case result of
        Ok users ->
          let
            dict = List.foldl (\u acc -> Dict.insert u.userId u.userNickname acc) Dict.empty users
          in
          ( { model | users = users, userDict = dict }, Cmd.none )
        Err err -> ( { model | errorMsg = Just (errorToString err) }, Cmd.none )
    ...
    [ div [ class "max-w-6xl mx-auto space-y-8" ]
      ...
      , div [ class "bg-white/90 backdrop-blur-lg rounded-3xl p-10 shadow-2xl" ]
        (case model.token of
          ...
          Just _ -> [ viewNavigation model, viewCurrentPage model ]
        )
      ]
    ]
...
viewHeader model =
  header [ class "text-center space-y-3" ]
    ...
    , case model.info of
      Just info ->
        div [] [ p [ class "text-xl text-slate-700" ] [ text (Maybe.map .webNotice model.meta |> Maybe.withDefault "千禧年代的数字化合租生活") ]
...
viewExpensesPage model =
  let total = List.foldl (\e acc -> acc + e.expenseAmount) 0 model.expenses in
  div [ class "space-y-8" ]
    [ div [ class "flex justify-between items-center" ]
